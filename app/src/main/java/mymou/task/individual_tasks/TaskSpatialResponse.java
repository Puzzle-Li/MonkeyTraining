...
public class TaskSpatialResponse extends Task {
    ...
    public void onViewCreated(final View view, Bundle savedInstanceState) {
        logEvent(TAG+" started", callback); //  数据display
    ...
    private void assignObjects() {
        ...
        for (int i = 0; i < cues.length; i++) {
            ...
            cues[i].setWidth(prefManager.cue_size/2);
            cues[i].setHeight(prefManager.cue_size/2);
...
//        }
    ...
    private void cuesLayout(Button[] cues) {
        Point centre = UtilsTask.locateCentre_screen(getActivity());
        for (int i=0;i<cues.length;i++) {
            float loc_x = (float) (centre.x + centre.x*4/5*Math.cos(2*3.14*i/cues.length));
            float loc_y = (float) (centre.y + centre.x*4/5*Math.sin(2*3.14*i/cues.length));
            cues[i].setX(loc_x - PreferencesManager.cue_size/2/2);
            cues[i].setY(loc_y - PreferencesManager.cue_size/2/2);
        }
    }
