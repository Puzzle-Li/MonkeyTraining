...
public class TaskManager extends FragmentActivity implements View.OnClickListener {
    ...
    // Task objects
    private static Button[] cues_Go = new Button[8]; // Go cues to start a trial    //TODO  cues_Go要设置在中心位置
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Create ui Elements
        assignObjects();
        initialiseScreenSettings(); //  对屏幕的全屏、隐藏导航面板做了初始化设置

        // Load settings
        loadAndApplySettings();
        loadtask(); //  找到要做的那个任务，这里是和Main menu的接口
    ...
    private void disableExtraRewardCues() {
        // Disable reward cues for extra monkeys
    ...
    private void assignObjects() {
        ...
        possible_cue_locs = new UtilsTask().getPossibleCueLocs(this);   //  通过计算屏幕大小和所需的cue的尺寸计算出本任务所需的一系列点，返回的是一个point[]
        trialData = new ArrayList<String>();
        ...
        // Layout views
        for (int i = 0; i < cues_Go.length; i++) {  //  这里一共加进去8个cues_Go
    ...
    private void positionGoCues() {
        ...
        if (preferencesManager.num_monkeys == 1) {
            UtilsTask.centreCue(cues_Go[0], this, preferencesManager.cue_size);
            return;
        }
